...
public class Activity2a_triplet {
                ...
                if (tokens[0].length() > 0 && tokens[1].length() > 0) {
                    ...
                    for (int i = 0; i < subWord.length; i++) {
                        for (int j = i + 1; j < subWord.length; j++) {
                            for (int k = j + 1; k < subWord.length; k++) {
                                if (i == j || k==j || k==i)
                                    continue;
                                if (subWord[i].trim().length() == 0)
                                    continue;
                                if (subWord[j].trim().length() == 0)
                                    continue;
                                if (subWord[k].trim().length() == 0)
                                    continue;

                                String w = new String();
                                String ch1 = subWord[i].toLowerCase().replaceAll("[^A-Za-z]+", "");
                                String ch2 = subWord[j].toLowerCase().replaceAll("[^A-Za-z]+", "");
                                String ch3 = subWord[k].toLowerCase().replaceAll("[^A-Za-z]+", "");
                                ...
                                ch3 = ch3.replaceAll("j", "i");
                                ch3 = ch3.replaceAll("v", "u");
                                ...
                                if (lemmaDict.containsKey(ch3)) {
                                    lemma3 = lemmaDict.get(ch3);
                                } else {
                                    lemma3.add(ch3);
                                ...
                                for (String var1 : lemma1) {
                                    for (String var2 : lemma2) {
                                        for (String var3 : lemma3) {

                                            String[] array = new String[]{var1, var2, var3};
                                            //set array equal to the inputs
                                            Arrays.sort(array);
                                            word = new Text();
                                            lemmaLoc = new Text();

                                            word.set(array[0] + ":" + array[1]+":"+array[2]);
                                            lemmaLoc.set(loc);
...
/*    public static class IntSumCombiner
    ...
    public static void main(String[] args) throws Exception {
        ...
        job.setJarByClass(Activity2a_triplet.class);
